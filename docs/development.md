...
| `nbdsl_kernel/tests/test_identity.py` | bare worker + installed kernelspec | wire-protocol gate: `ready.protocol` must match the adapter's `WIRE_PROTOCOL`; a mismatch refuses cells as typed `WireProtocolError` |
