...
import { makeLean4Parser, DEFAULT_DSL_KEYWORDS } from "./lean4";
import { makeCasDslParser } from "./casdsl";
...
const casdslLanguagePlugin: JupyterFrontEndPlugin<void> = {
  id: "jupyterlab_nbdsl:casdsl-language",
  description: "CasDsl syntax highlighting for casdsl notebook cells",
  autoStart: true,
  ...
  activate: (_app: JupyterFrontEnd, languages: IEditorLanguageRegistry) => {
    languages.addLanguage({
      ...
      mime: "text/x-casdsl",
      extensions: ["casdsl"],
      support: new LanguageSupport(StreamLanguage.define(makeCasDslParser())),
    });
  },
};
